//-----------------------------------------------------------
class StingerFactory extends ONSRVFactory;
...
Stinger(TheTarget).FD_BoostDelayTime=FD_BoostDelayTime;
Stinger(TheTarget).FD_VehicleKickMass=FD_VehicleKickMass;
Stinger(TheTarget).FD_ScanRange=FDScanRange;
Stinger(TheTarget).FD_ReControlTime=FD_ReControlTime;
Stinger(TheTarget).FD_Boosts=FD_Boosts;
Stinger(TheTarget).FD_bUseVocalCount=FD_bUseVocalCount;
Stinger(TheTarget).FD_bBoosterEnabled=FD_bBoosterEnabled;
Stinger(TheTarget).FD_BoostFXEnabled=FD_BoostFXEnabled;
Stinger(TheTarget).FD_HardAttachFX=FD_HardAttachFX;
Stinger(TheTarget).FD_FirstEntryEvent=FD_FirstEntryEvent;
Stinger(TheTarget).FD_AppliedBoostForce=FD_AppliedBoostForce;
Stinger(TheTarget).FD_Groundspeed=FD_Groundspeed;
Stinger(TheTarget).FD_ScanRange=FDScanRange;
...
for (I=0;I<FD_TheFx.Length;I++){Stinger(TheTarget).FD_TheFx[I]=FD_TheFx[I];}
for (I=0;I<FD_TheSound.Length;I++){Stinger(TheTarget).FD_TheSound[I]=FD_TheSound[I];}
...
defaultproperties
{
     ...
     VehicleClass=Class'VecTech.Stinger'
     AmbientGlow=51
}
