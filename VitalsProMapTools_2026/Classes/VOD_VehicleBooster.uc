...
var() bool                        bUseBoosterSoundFX;
Var() bool                        bBoostFlyingVehicles;
Var() float                       Capturetime;
var() sound                       BoosterSoundFX;
var() bool                        bAirbornBoost;
var   float                       TimerFrequency;

simulated function Touch( Actor Other )
{
 if ( Other.IsA('OnsVehicle') && ( OnsVehicle(Other).bVehicleOnGround || bBoostFlyingVehicles ) )
    {
...
simulated function VecBoost( OnsVehicle VehicleToBoost )
{
 ...
 if ( SoundFxBoost != None )
    {
     PlayOwnedSound( SoundFxBoost );                 // sound fx
     VehicleToBoost.MakeNoise(1.0);
    }


if (bUseBoosterSoundFX == true && VehicleToBoost.Driver != None && PlayerController(VehicleToBoost.Driver.Controller) != None)
{
    if (FRand() < 0.5)
    {
      if (SoundFxBoost != none)
         PlayerController(VehicleToBoost.Driver.Controller).ClientPlaySound(SoundFxBoost);
    }
    else
    {
        if (BoosterSoundFX != none)
        PlayerController(VehicleToBoost.Driver.Controller).ClientPlaySound(BoosterSoundFX);
    }
}
...
{
     bCollideActors=True
     bUseBoosterSoundFX=true
     bBoostFlyingVehicles=False
     ...
     BoosterSoundFX=Sound'AnnouncerFemale2K4.Generic.Booster'
