...
class VitalsVehicleToolbox extends Actor;
...
function Timer()
{
    ...
    // Count down BotLock entries, release when expired
...
function ApplyVehicleImpulse( ONSVehicle TargetVehicle, Actor DirectionSource )
{
    ...
    if ( bDirectional && DirectionSource != None )
    {
    ...
    TargetVehicle.KAddImpulse(
        AppliedBoostForce >> RotRelation.Rotation,
        PointOfForce >> RotRelation.Rotation
    );
}

function StabiliseAircraft( Pawn VehiclePawn )
{
...
simulated function Emitter SpawnRndEmitter( Vector SpawnPos, array<class<Emitter> > EmitterPool, Actor HardAttachedTo )
{
    ...
    if ( SpawnedEmitter != None && HardAttachedTo != None )
    {
